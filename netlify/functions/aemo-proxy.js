 ...
 *
 * Actuals     → OpenElectricity REST API (last 12 hours, 5-min intervals)
 * P5 Forecast → NEMWeb P5_Reports ZIP   (~1 hour ahead, 5-min intervals)
 * PD Forecast → NEMWeb Predispatch_Reports ZIP (~12 hours ahead, 30-min intervals)
 *
 * ZIP download uses 2 Range requests (EOCD approach):
...
const PD_DIR  = NEMWEB + '/REPORTS/CURRENT/Predispatch_Reports/';
...
/* ── OpenElectricity: last 12 hrs of actuals ────────────────────────── */
async function fetchActuals(region, apiKey) {
  const since = new Date(Date.now() - 12 * 3600 * 1000).toISOString().slice(0, 19);
...
function resolveZipUrl(html, pattern) {
  ...
  if (!hits.length) throw new Error('No ZIP matching "' + pattern + '" in NEMWeb listing');
  hits.sort();
...
async function downloadAndUnzip(zipUrl, timeoutMs) {
  ...
  if (tailRes.status !== 206) {
    ...
    const buf = Buffer.from(await (tailRes.ok ? tailRes : fetchWith(zipUrl, { headers: hdrs }, 20000)).arrayBuffer());
  ...
  const tailFileOff = Math.max(0, totalSize - 4096);
  ...
  const cdOffset  = tail.readUInt32LE(eocdBufPos + 16);
  const cdBufPos  = cdOffset - tailFileOff;
  ...
  const lhOffset   = tail.readUInt32LE(cdBufPos + 42);
  ...
  const dataOff  = 30 + lhFnLen + lhExLen;
...
async function fetchP5Forecast(region) {
  ...
  return allRows
    ...
    .map(r => {
      ...
      const dt = (r.INTERVAL_DATETIME || '').replace(/\//g, '-');
...
/* ── Predispatch forecast fetch (~12 hours, 30-min intervals) ───────── */
async function fetchPDForecast(region) {
...
exports.handler = async function (event) {
  ...
  // Fetch actuals + P5 forecast + Predispatch forecast in parallel
  const [aRes, p5Res, pdRes] = await Promise.allSettled([
    fetchActuals(region, apiKey),
    fetchP5Forecast(region),
    fetchPDForecast(region),
  ]);

  if (aRes.status  === 'rejected') console.error('[aemo-proxy] actuals:', aRes.reason?.message);
  if (p5Res.status === 'rejected') console.warn( '[aemo-proxy] P5:', p5Res.reason?.message);
  if (pdRes.status === 'rejected') console.warn( '[aemo-proxy] PD:', pdRes.reason?.message);

  const actuals    = aRes.status  === 'fulfilled' ? aRes.value  : [];
  const p5Forecast = p5Res.status === 'fulfilled' ? p5Res.value : [];
  const pdForecast = pdRes.status === 'fulfilled' ? pdRes.value : [];

  // Expose errors in response headers for frontend diagnostics
  cors['Access-Control-Expose-Headers'] = 'X-Forecast-Error,X-Forecast-Debug';
  const forecastErrors = [
    p5Res.status  === 'rejected' ? 'P5: '  + (p5Res.reason?.message  || 'unknown') : null,
    pdRes.status  === 'rejected' ? 'PD: '  + (pdRes.reason?.message  || 'unknown') : null,
  ].filter(Boolean);
  ...
  if (!actuals.length && !p5Forecast.length && !pdForecast.length) {
    const msg = [
      ...
      p5Res.status === 'rejected' ? 'P5: '       + p5Res.reason?.message : null,
      pdRes.status === 'rejected' ? 'PD: '       + pdRes.reason?.message : null,
    ].filter(Boolean).join(' | ');
    return { statusCode: 502, headers: cors, body: JSON.stringify({ error: msg }) };
  }

  // Merge: PD first, then P5 (finer resolution wins at overlaps), then actuals (always win)
  const byTime = {};
  [...pdForecast, ...p5Forecast, ...actuals].forEach(r => { byTime[r.intervalDatetime] = r; });
  const merged = Object.values(byTime).sort((a, b) =>
    a.intervalDatetime < b.intervalDatetime ? -1 : 1);

  console.log('[aemo-proxy] ' + region + ': '
    + actuals.length + ' actual + '
    + p5Forecast.length + ' P5 + '
    + pdForecast.length + ' PD → '
    + merged.length + ' total');
